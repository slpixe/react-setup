...
const viteCreator: Record<PackageManager, (name: string) => string> = {
  npm: (name) => `npm create vite@latest ${name} -- --template react-ts`,
  pnpm: (name) => `pnpm create vite ${name} --template react-ts`,
  yarn: (name) => `yarn create vite ${name} --template react-ts`,
  bun: (name) => `bun create vite ${name} --template react-ts`,
}
...
export function buildManualSteps(options: SetupOptions): ManualStep[] {
  ...
  return [
    {
      title: 'Start with Vite, React, and TypeScript',
      description: options.mode === 'new'
        ? 'Create the basic React TypeScript application first. The following steps add each opinionated layer separately.'
    ...
    {
      title: 'Enable stable React Compiler',
      description: 'Vite 8 and @vitejs/plugin-react 6 use the compiler preset through Rolldown’s Babel bridge. The older react({ babel: … }) form no longer applies. For React 17 or 18, pass the matching target and add react-compiler-runtime.',
      references: [
      ...
      snippets: [
        ...
        },
        {
          ...
          code: `import react, { reactCompilerPreset } from '@vitejs/plugin-react'\nimport babel from '@rolldown/plugin-babel'\nimport { defineConfig } from 'vite'\n\nexport default defineConfig({\n  plugins: [react(), babel({ presets: [reactCompilerPreset()] })],\n})`,
        },
      ],
    },
    {
